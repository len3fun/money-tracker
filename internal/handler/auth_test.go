...
func TestHandler_signIn(t *testing.T) {
	type mockBehavior func(s *mock_service.MockAuthorization, inputUser signInInput)
	...
	testTable := []struct {
	...
	}{
		{
			...
			mockBehavior: func(s *mock_service.MockAuthorization, inputUser signInInput) {
				s.EXPECT().GenerateToken(inputUser.Username, inputUser.Password).Return("token", nil)
			},
			expectedStatusCode:  200,
		...
		},
		{
			...
			mockBehavior: func(s *mock_service.MockAuthorization, inputUser signInInput) {
				s.EXPECT().GenerateToken(inputUser.Username, inputUser.Password).Return("", errors.New("service failure"))
			},
			expectedStatusCode:  500,
			expectedRequestBody: `{"message":"service failure"}`,
		},
